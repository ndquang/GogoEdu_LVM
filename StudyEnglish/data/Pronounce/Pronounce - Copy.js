...
{	
	...
	_bTestAndCreat = false;
	 s_content = GetTextFromID("ID_TEXT_CONTENT") ;
	arrayOfLines = s_content.match(/[^\r\n]+/g);
	cntQst = length(arrayOfLines);
...
function  CreateQuestion()
{
 ...
 if(_index>=cntQst)
    ...
    var strCh = arrayOfLines[_index];
    aWords =  splitCau(strCh);
	SetText("","help",aWords[0]);
    cntWord = length(aWords); 	
	for(var i= 1; i< 5;i++)
	{
		SetText("","w_"+i,aWords[i]);
