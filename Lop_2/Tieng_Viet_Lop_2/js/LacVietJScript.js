...
function SetRsc(pageName, objName, newID) {
 ...
 //   var googleTTS = new window.GoogleTTS();
    function SpeakEN(page, object, text) {
       /* googleTTS.play('"'+text.toString()+'"', 'en', function (err) {
            if (err) {
     ...
     function SpeakVN(pageName, objName, text, nostopsound, english) {
     /*    var googleTTS = new window.GoogleTTS('vi');
         googleTTS.play("'" + text + "'", 'vi', function (err) {
             if (err) {
                 console.log(err.toString());
             }
             console.log('Finished playing');
    ...
    return point;
        ...
        function SetPaint(page, object, b_Paint) {
            ...
            if (b_Paint == 1)
             {
                 curObject.on('mouseup touchend dragend', function (evt)/*---dragend---*/
                 {
                     m_pgObjCaller = this;
              
                     PaintUpObj(stage.getMousePosition().x, stage.getMousePosition().y, curObject.getName());
                     return;
                 }
                 );
                 curObject.on('mousedown touchstart', function (evt) {
                     m_pgObjCaller = this;
                  
                         PaintDownObj(stage.getMousePosition().x, stage.getMousePosition().y, curObject.getName(), 40);
                     return;
                 }
                 );
                curObject.on('mousemove touchmove dragmove',function(evt)
                {
                    m_pgObjCaller = this;
             
                    PaintMoveObj(stage.getMousePosition().x, stage.getMousePosition().y, curObject.getName());
                  return;
                }
              );
            }
    ...
    function ShapeType(page, object, m_shape) {
        ...
        if (typeof m_shape === 'undefined') {
            return curObject.attrs.shapeType;
             ...
             curObject.attrs.shapeType = m_shape;
        }
    }
