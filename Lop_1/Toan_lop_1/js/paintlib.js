...
function getImageData(pageName, objectName) {
    ...
    x_obj = object.getX() * m_dZoomScale;
    y_obj = object.getY() * m_dZoomScale;
    width = object.getWidth() * m_dZoomScale;
    height = object.getHeight() * m_dZoomScale;
    imageData = context.getImageData(x_obj, y_obj + curentPage.getY() * m_dZoomScale, width, height);
}
function putImageData(pageName, objectName) {
    ...
    width = object.getWidth() * m_dZoomScale;
    height = object.getHeight() * m_dZoomScale;
    context.putImageData(imageData, x_obj, y_obj + curentPage.getY() * m_dZoomScale, 0, 0, width, height);
...
function PaintDownObj(x, y, objectName, tolerance) {
    ...
    if (object.getPaint() == false)
        return;
    x_obj = parseInt(object.getX() * m_dZoomScale); //  + (wclient- stage.getWidth() / 2); khong the bo parseInt
    y_obj = parseInt(object.getY() * m_dZoomScale); //   + (hclient- stage.getHeight() / 2 ); khong the bo parseInt
    width = parseInt(object.getWidth() * m_dZoomScale); //khong the bo parseInt
    height = parseInt(object.getHeight() * m_dZoomScale); //khong the bo parseInt
    var context = object.getContext();
...
/// 6/12/2015
var started = false;
var x, y, startx, starty, cx, cy, b_fill = true;
var data = [];
var ppts = []; //for pen
// draw grid
...
//    contextp = canvasp.getContext('2d');
    ...
    canvas.setAttribute("style", "position: absolute; left: " + x_obj + "px; top: " + y_obj + "px; cursor: crosshair");
    object.tempCanvas = canvas
    //stage.content.appendChild(object.tempCanvas);
    // bufferCanvas
    //stage.content.appendChild(canvas);
    ...
    object.on('mousedown touchstart', function (evt) {
        m_pgObjCaller = this;
        if (curentCanvas)
        ...
        x_obj = parseInt(m_pgObjCaller.getX() * m_dZoomScale);
        y_obj = parseInt(m_pgObjCaller.getY() * m_dZoomScale);
...
function update() {
    contextp.drawImage(curentCanvas, x_obj, y_obj);
